...
export default function SignInPage() {
  ...
  useEffect(() => {
    if (!loading && user) {
      router.push(user.isAdmin ? '/admin' : '/order');
    }
  }, [user, loading, router]);
    ...
    <div className="flex flex-1 items-center justify-center p-4 bg-background">
      <Card className="grid w-full max-w-4xl grid-cols-1 overflow-hidden md:grid-cols-2">
         <div className="hidden md:flex flex-col items-center justify-center bg-primary p-12 text-center text-primary-foreground">
            ...
            <h2 className="mt-6 text-3xl font-bold font-headline">Welcome Back!</h2>
            <p className="mt-2 text-primary-foreground/80">Ready to order some fresh bread? Sign in to continue where you left off.</p>
        </div>
        <div className="p-8">
